...
기본 구성은 **총 160문제, 약 3시간**입니다. 방을 만든 뒤 설정 화면에서
...
속담·사자성어 한 문제는 60초입니다.

속담·사자성어 라운드에서는 두 가지가 라운드 도중에 방 전체에 공개됩니다.
**남은 시간 30초**가 되면 힌트가 한 번 뜨고(사자성어는 정답의 초성 네 글자,
속담은 문제 파일에 적어 둔 핵심 낱말), 정답자가 나올 때마다
`1등 - 닉네임` 처럼 순위가 바로 뜹니다. 오답이나 남이 적은 답은 어느 쪽에도
나오지 않습니다. 노래 라운드는 예전 그대로, 정답 공개 전까지 아무것도 알려
주지 않습니다.
...
비공개 배포이며 Sites 버전 1(`a8277ad`)의 **초기 UI 프로토타입**만 보여줍니다.
그 뒤 서버와 클라이언트는 `main`에 통합됐지만 이 주소에는 다시 배포되지
않았습니다. 현재 배포에는 게임 서버도 연결되어 있지 않아 여러 사람이 같은
방에서 함께 플레이할 수 없습니다. 위 [실행](#실행) 절차로 띄우는 완성형과
다른 물건입니다. 다시 배포하려면 먼저 별도 공개 게임 서버와
`VITE_GAME_SERVER`를 준비해야 합니다.
...
| **구현됨 (클라이언트)** | 방 만들기 화면의 진행 순서 안내와 챕터별 문제 수 입력, 대기실의 구성 표시, 속담·사자성어 라운드의 단서 카드와 진행 표시, 남은 30초 힌트와 실시간 득점자 피드, 정답 공개 시 빠진 부분·한자·뜻풀이와 득점자 순서, 방 만들기·참여·닉네임, 대기실과 준비 상태, 서버 기준 카운트다운·타이머, 클립·유튜브 재생, 정답 입력과 본인 전용 판정 결과, 방장 일시정지/재개/스킵, 정답 공개, 실시간 순위와 최종 시상대, 새로고침 후 이어하기 |
...
| **미구현** | 방 상태 영속화(프로세스 재시작 시 소멸), 여러 실기기에서의 유튜브 자동 재생 검증, 음원 파일 방식용 라이선스 음원, Sites용 별도 공개 게임 서버 |

기능별 상세는 [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md) §5,
완료된 브랜치 통합 기록은 [`docs/integration-plan.md`](docs/integration-plan.md)에
있습니다.
