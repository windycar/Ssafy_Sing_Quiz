...
방을 만들 때 세 가지 모드 중 하나를 고릅니다.

| 모드 | 화면에 나오는 것 | 정답으로 인정되는 것 | 문항 수 | 득점 |
| --- | --- | --- | --- | --- |
| **노래 맞히기** | 방장이 튼 하이라이트 (최대 1분) | 곡 제목 | 최대 100곡 | 먼저 맞힌 **1명**이 1점, 그 즉시 라운드 종료 |
| **속담 맞히기** | 속담의 앞부분 | 뒷부분, 또는 속담 전체 | 30문제 | 먼저 맞힌 **3명까지** 각 1점 |
| **사자성어 맞히기** | 뜻풀이 | 한글 네 글자, 또는 한자 네 글자 | 30문제 | 먼저 맞힌 **3명까지** 각 1점 |

속담·사자성어는 서버가 가진 50문항 은행에서 방마다 30문항을 새로 뽑아 섞습니다.
같은 저녁에 두 팀이 같은 모드를 해도 문제 구성이 달라집니다.
...
처음 실행하면 이 폴더에 파일 세 개가 생깁니다. **이 세 개만 고치면 됩니다.**
...
| 모드 공용 문제 모델 (노래·속담·사자성어) | [`shared/questions.ts`](shared/questions.ts) | 구현·테스트 완료. 라운드 엔진은 모드를 구분하지 않습니다 |
...
없습니다. 서버만 켜고 방을 만들 때 모드를 고르면 됩니다.

```bash
node server/main.ts --songs data/songs.recovered.json
```

브라우저로 <http://localhost:8787> 을 열고 **방 만들기** 옆에서
`속담 맞히기` 또는 `사자성어 맞히기` 를 고른 뒤 방을 만듭니다. 곡 카탈로그는
노래 모드에서만 쓰이므로 이 두 모드에서는 화면에 나오지 않습니다.

> `--songs` 는 노래 모드를 함께 쓸 때만 의미가 있습니다. 속담·사자성어만 할
...
| 문서 | 내용 |
| --- | --- |
| [`docs/GAME_RULES.md`](docs/GAME_RULES.md) | 게임 방식 — 모드별 규칙, 라운드 길이, 점수·순위, 정답 판정 |
...
| 구분 | 내용 |
| --- | --- |
| **구현됨 (게임 모드)** | 방 생성 시 노래·속담·사자성어 중 선택, 방에 모드 기록, 모드별 라운드 길이와 득점 자리 수, 방마다 새로 섞는 문제 순서 |
...
| **구현됨 (클라이언트)** | 방 만들기 화면의 모드 선택과 대기실의 모드 표시, 속담·사자성어 라운드의 단서 카드와 `현재 문제 / 30` 진행 표시, 정답 공개 시 빠진 부분·한자·뜻풀이와 득점자 순서, 방 만들기·참여·닉네임, 대기실과 준비 상태, 서버 기준 카운트다운·타이머, 클립·유튜브 재생, 정답 입력과 본인 전용 판정 결과, 방장 일시정지/재개/스킵, 정답 공개, 실시간 순위와 최종 시상대, 새로고침 후 이어하기 |
