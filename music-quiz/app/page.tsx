...
export default function Home() {
  ...
  const onCreateRoom = async (): Promise<void> => {
    ...
    try {
      ...
      // host token until the room exists.
  ...
  const onSaveSetlist = async (): Promise<void> => {
    ...
    setBusy(true);
    try {
      const setlist = await setSetlist(roomId, hostToken, { songCount, media }, apiBase());
      setSetlistNote(
        setlist.songCount === 0
          ? "재생 가능한 곡이 없습니다. 음원을 등록해 주세요."
          : `${setlist.songCount}곡으로 진행합니다. (재생 가능 ${setlist.playableCount}곡` +
  ...
  const onJoin = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (!room.ready) setToast("방장이 아직 곡을 고르는 중입니다.");
    } catch (error) {
          ...
          <div className="lobby-grid">
            ...
            <aside className="lobby-side">
              <section className="panel settings-card">
                ...
                <p>방장은 곡을 고르고 게임을 진행합니다. 참가자에게는 참여 코드만 알려 주세요.</p>
              </section>
        ...
        <section className="game-layout page-enter">
          ...
          <section className="stage">
            <div className="round-meta">
              <div>
                <span>ROUND</span>
                <b>{String((round?.song.index ?? 0) + 1).padStart(2, "0")}</b>
                <i>/</i>
                <strong>{round?.song.totalSongs ?? 0}</strong>
              </div>
            ...
            {state?.phase === "REVEAL" && state.reveal !== null ? (
              <div className="answer-reveal">
                <span>{state.reveal.winner !== null ? "FIRST CORRECT ANSWER" : "ROUND ANSWER"}</span>
              ...
              <div className="question-copy">
                <span className="genre-pill">K-POP</span>
                <h2>이 노래의 제목은?</h2>
                <p>{audioNote !== "" ? audioNote : "하이라이트를 듣고 정답을 입력하세요"}</p>
                {audioNote !== "" && (
                  <button
                    className="text-button"
            ...
            <form className="answer-form" onSubmit={onSubmitAnswer}>
              ...
              <div>
                <input
                  ...
                  placeholder={state?.phase === "IN_ROUND" ? "노래 제목을 입력하세요" : "라운드 대기 중"}
...
function Feedback({ state }: { state: ClientState | null }) {
